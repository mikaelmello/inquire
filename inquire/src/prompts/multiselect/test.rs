...
fn first_option_renders_on_new_line_without_filtering() {
...

    {
        ...
        let _ans = MultiSelect::new("Select rating", options)
            .without_filtering()
            .prompt_with_backend(&mut backend)
            .unwrap();
    }
...
#[test]
// Test for multiline option indentation in multiselect
fn multiline_options_are_properly_indented() {
    ...
    {
        let mut backend = Backend::new(input_reader, terminal, render_config).unwrap();
        let _ans = MultiSelect::new("Choose people:", options)
            .without_filtering()
            .prompt_with_backend(&mut backend)
            .unwrap();
    }
    ...
    // The first option should be selected (highlighted with ">")
    match_text(&mut output, "?");
    match_text(&mut output, " ");
    match_text(&mut output, "Choose people:");
    match_text(&mut output, " ");
    match_text(&mut output, "\r");
    match_text(&mut output, "\n");
    
    // First option (selected, should have ">" prefix and "[ ]" checkbox)
    match_text(&mut output, ">");
    match_text(&mut output, " ");
    match_text(&mut output, "[ ]");
    match_text(&mut output, " ");
    ...
    match_text(&mut output, "\r");
    match_text(&mut output, "\n");
    match_text(&mut output, "      "); // Indentation for second line (6 spaces: ">" + " " + "[ ]" + " ")
    ...
    // Second option (not selected, should have " " prefix and "[ ]" checkbox)
    match_text(&mut output, " ");
    match_text(&mut output, " ");
    match_text(&mut output, "[ ]");
    match_text(&mut output, " ");
