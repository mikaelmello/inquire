...
use self::{actions::DateSelectAction, config::DateSelectConfig};
...
impl<'a, B> PromptTrait<B, DateSelectConfig, DateSelectAction, NaiveDate> for DateSelectPrompt<'a>
where
    B: DateSelectBackend,
{
    fn message(&self) -> &str {
    ...
    fn handle(&mut self, action: DateSelectAction) -> InquireResult<()> {
        match action {
